...
async def analyze_face_full(payload: FaceAnalyzePayload):
    try:
        ...
        short_prompt = f"""
        จากผลคะแนนผิวต่อไปนี้ ช่วยสรุปข้อมูลแบบย่อในรูปแบบ JSON เพียงอันเดียว
        โดยต้องมี 3 ฟิลด์ดังนี้ และตอบเป็นภาษาไทย:

        1) "summary": ประโยคสรุปภาพรวมผิว 1 บรรทัด เช่น
           "ผิวโดยรวมค่อนข้างดี แต่มีจุดด่างดำเล็กน้อยและผิวค่อนข้างแห้ง"
        2) "highlights_short": ลิสต์ข้อความสั้น ๆ 1–3 ข้อ
           บอกจุดเด่นของผิว (เช่น "ผิวเรียบเนียนดี", "ริ้วรอยยังไม่ชัด")
        3) "improvements_short": ลิสต์ข้อความสั้น ๆ 1–3 ข้อ
           บอกสิ่งที่ควรปรับปรุง (เช่น "มีรอยสิวเล็กน้อย", "โทนสีผิวไม่สม่ำเสมอ")

        คะแนนรวม: {result['overall_score']}/100
        คะแนนรายด้าน (ยิ่งสูงคือยิ่งดี): {result['dimension_scores']}
        โปรไฟล์: เพศ={sex}, อายุ={age_range}, ผิว={skin_type}, แพ้ง่าย={"ใช่" if sensitive else "ไม่ใช่"}, ความกังวล="{concerns or "ไม่ระบุ"}"

        ให้ตอบเป็น JSON เท่านั้น ห้ามมีข้อความอื่นนอกจาก JSON
        ตัวอย่างรูปแบบ (ตัวอย่างเท่านั้น อย่าใช้ค่าตัวเลขตามนี้):

        {{
          "summary": "ผิวโดยรวมดีแต่มีจุดด่างดำเล็กน้อย",
          "highlights_short": ["ผิวเรียบเนียนและชุ่มชื้นดี"],
          "improvements_short": ["มีรอยแดงเล็กน้อยและจุดด่างดำ"]
        }}
        """
